Feature: Procesar deducible
    ...
    Scenario Outline: Póliza con deducible texto plano con errores
        Given la póliza tiene un deducible en forma del <texto>
        When ejecutamos el conversor de deducible
        Then obtenemos un error por la parametrización del deducible en <detalle>

        Examples:
            | texto      | detalle        |
            | inputError | RespuestaError |
